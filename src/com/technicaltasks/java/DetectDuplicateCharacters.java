package com.technicaltasks.java;

public class DetectDuplicateCharacters {

	public static void main(String[] args) {

		String stringData = "Java hooks";
		int count;
		
		System.out.println("The entered string is: " + stringData);
		
		// Converts given string into character array
		char string[] = stringData.toCharArray();

		System.out.println("Duplicate characters in given string are: ");
		
		// Count the frequency of each character present in the string
		for (int i = 0; i < string.length; i++) {
			count = 1;
			for (int j = i + 1; j < string.length; j++) {
				if (string[i] == string[j] && string[i] != ' ') {
					count++;
					// Set string[j] to 0 to avoid printing visited character
					string[j] = '0';
				}
			}
			// Character is considered as duplicate if count is greater than 1
			if (count > 1 && string[i] != '0')
				System.out.println(string[i]);
		}

	}

}
